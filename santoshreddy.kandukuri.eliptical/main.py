...
def plotcurve(m,a,b,x1,y1,x2,y2,x3,y3):
#Determines width and height of plot
    w = max([abs(x1),abs(x2),abs(x3)])+10
    h = max([abs(y1),abs(y2),abs(y3)])+10

# Annotate the plot with your name using width (w) and height (h) as your reference points.
    an1 = plt.annotate("Eliptical curve", xy=(-w+2 , h-2), xycoords="data",
              va="center", ha="center",
